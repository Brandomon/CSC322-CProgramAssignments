...
void eval(char *cmdline) 
{
    ...
    if (!builtin_command(argv))                     /* No need to fork buildin command */
    {
        ...
        if ((pid = fork()) == 0)
        {
            ...
            // Put the child in a new process group, GID = PID
            setpgid(0,0);

            // If execve is less than 0, display error message and exit
...
void do_bgfg(char **argv) 
{
    ...
    // If id is NULL, print error message
    if(id == NULL)
    {
        // Command requires pid or %jobid argument error message
    ...
    if(id[0] == '%') 
    {
        ...
        // Set length to loop through each character of ID
        length = strlen(id);

        // For each character of ID, check for digits
        for (i = 0; i < length; i++)
        {
            // If ID is not a digit number, print error message
            if(!isdigit(id[i]))
            {
                // Argument must be a PID or %jobid error message
        ...
        // If job is NULL, print error message
        if(job == NULL)
        {
            // No such job error message
    ...
    {
        // Set length to string length of ID
        length = strlen(id);
    ...
    // Send SIGCONT to the job using kill function
    kill(-(job->pid), SIGCONT);

    // If command is foreground (fg)
    if(!strcmp(argv[0], "fg"))
    {  
        // Wait until foreground job terminates
        job->state = FG;
        waitfg(job->pid);
    }
...
void waitfg(pid_t pid)
{
    ...
    while (1)
    {
        // If pid is no longer the foreground process
        if (pid != fgpid(jobs))
        {
            // If verbose, print message
            if (verbose) 
            {
                // Process no longer foreground process message
        ...
        // Else pid is foreground process
        else
        {
            // Block until process pid is no longer the foreground process using sleep function
            sleep(1);
        }
    }
...
void sigchld_handler(int sig) 
{
    ...
    // If PID is less than 0 AND errno is not equal to ECHILD
